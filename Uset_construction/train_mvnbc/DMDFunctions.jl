...
function DMD_ellipsoid(X::Matrix, n_clusters::Int64, e::Float64; max_iteration::Int64=10)
    clusters, π_k = initialize_clusters(X, n_clusters, e)
    K = n_clusters
    n = size(X, 2)
    last_log_p = 0
    ...
    # Initialize a matrix to store the volumes of each cluster at each iteration
    cluster_vols = zeros(max_iteration, K)
...
function initialize_clusters(X, n_clusters, e; r=4)
    d, n = size(X)
    ...
    for k in 1:n_clusters
        ...
        for i in 1:n
            L[i, k] = (X[:, i] - x_hat)' * H * (X[:, i] - x_hat) / d <= 1 + TOLERANCE
        end
    ...
    mixing_coeffs = sum(L, dims=1) ./ sum(L) .* (1 - e)
    
    return MVCEs.MVBCResult(Hs, x_hats, L, vols), mixing_coeffs
end

function initialize_clusters_randomly(X, n_clusters, e, rndidx; r=4)
    d, n = size(X)
    ...
    for k in 1:n_clusters
        x = X[:, rand(Random.MersenneTwister(rndidx), 1:n, d+1)]
...
function dist_clusters(X, clusters)
    ...
    for k in 1:K
        ...
        for i in 1:n
            ...
            dist[i, k] = (X[:, i] - x_hat)' * H * (X[:, i] - x_hat) / d
        end
    end
...
function expectation_step(X, clusters, mixing_coeffs)
    ...
    for k in 1:K
        for i in 1:n
            gamma_nk[i, k] = mixing_coeffs[k] / clusters.vols[k] * exp(-0.5 * (dist[i, k]))
        end
    end
...
function maximization_step(X, clusters, gamma_nk, e, r=1)
    ...
    # println("gamma_nk: ", gamma_nk)
    weights = [quantile(Chisq(d), 1 - p) for p in gamma_nk]
    
    Hs = Matrix{Float64}[]
    x_hats = Vector{Float64}[]
    vols = []
    for k in 1:K
        H, x_hat, vol = MVCEs.minimum_volume_ell_weighted(X, weights[:, k])
        for i in 1:n
            L[i, k] = (X[:, i] - x_hat)' * H * (X[:, i] - x_hat) / d <= 1 + TOLERANCE
        end
