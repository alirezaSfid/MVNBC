...
function local_search_box(X::Matrix, clusters, e::Float64; max_iteration::Int64 = 100)
    ...
    n_candidatesMax = compute_dist_clusters(X, clusters, e)/2
...
function manage_overlap_points(X, clusters)
    ...
    for j = 1:n_clusters - 1
        for k = j+1:n_clusters
            ...
            shrinkedClusters1 = MVBBs.minimum_volume_box_labeled(X, L_shrinked1)
            Ovals1 = sum(shrinkedClusters1.vols)

            shrinkedClusters2 = MVBBs.minimum_volume_box_labeled(X, L_shrinked2)
...
function find_closest_to_one_indices(matrix, num_elements)
    ...
    return row_indices[sorted_indices[1:min(end, num_elements)]]
...
function manage_boundary_points(X, clusters, n_candidates, e)
    ...
    while neededPoint > 0
        ...
        shrinkedClusters = MVBBs.minimum_volume_box_labeled(X, L_shrinked)
        ...
        # println("neededPoint: $neededPoint")
        ...
        for j in 1:n_clusters
            ...
            clusters = MVBBs.minimum_volume_box_labeled(X, L_copy)
            cluster_vols_sum = sum(clusters.vols)
            # println("cluster_vols_sum: $cluster_vols_sum")
